"""E-commerce plugin demo data — idempotent seeder."""
import logging
...
def populate(app=None):
    """Populate shop demo data (idempotent)."""
    ...
    # --- Warehouse ---
    warehouse = db.session.query(Warehouse).filter_by(slug="main-warehouse").first()
    if not warehouse:
        warehouse = Warehouse(
            id=uuid4(),
            name="Main Warehouse",
    ...
    if created_count:
        db.session.commit()
        logger.info("[shop] populate_db: created %d records", created_count)
    else:
        logger.info("[shop] populate_db: all data already exists")

    # ── CMS Layouts, Widgets, Pages ──────────────────────────────────
    _populate_cms_content()

    # ── Email Templates ──────────────────────────────────────────────
    _populate_email_templates()


def _populate_cms_content():
    ...
    widget_cart_badge, _ = _get_or_create(
        CmsWidget,
        "cart-badge",
        name="Cart Badge",
        widget_type="vue-component",
