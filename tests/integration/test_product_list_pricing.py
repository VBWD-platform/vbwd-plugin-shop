...
Contract:
- ``GET /shop/products`` returns ``{"products": [...]}`` where each product carries
  a ``pricing`` block identical in shape/values to the detail route's block for the
  same product (net/gross + ``effective_display_mode`` + ``prices_display_mode``).
...
def test_list_products_each_item_has_pricing_block(db, client):
    ...
    listed = next(p for p in body["products"] if p["slug"] == product.slug)
...
def test_list_pricing_matches_detail_block(db, client):
    ...
    listed = next(
        p for p in list_resp.get_json()["products"] if p["slug"] == product.slug
    )
...
def test_list_netto_override_surfaces_effective_netto(db, client):
    ...
    assert resp.status_code == 200, resp.get_json()
    listed = next(p for p in resp.get_json()["products"] if p["slug"] == product.slug)
    pricing = listed["pricing"]
    assert pricing["prices_display_mode"] == "brutto"
