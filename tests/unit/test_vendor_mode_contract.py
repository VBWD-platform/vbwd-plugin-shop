...
The money path is decoupled: shop stamps the buyer invoice line with a LOCAL
key literal and the central ``marketplace`` plugin credits the selling vendor
from it — shop never imports marketplace. These tests pin the literal (so the
value can never drift from the documented ``vendor_id`` convention) and prove
shop's source names no ``plugins.marketplace`` import.
"""
import os
...
def test_shop_source_does_not_import_marketplace():
    offenders = []
    ...
    assert not offenders, (
        "Shop must not depend on the marketplace plugin — keep the money path "
        f"decoupled (stamp a literal, never import): {offenders}"
    )
